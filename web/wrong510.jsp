...
<body>
输入信息有误，请重新输入:-)
<c:if test="${pushN_null==1}">
    <br>发生模块：发送通知
    <br>错误信息：关键信息缺失，请检查表单输入是否完整
</c:if>
<c:if test="${alterC_null==1}">
    <br>发生模块：更改合同
    <br>错误信息：关键信息缺失，请检查表单输入是否完整
</c:if>
<c:if test="${alterPrice_wrong==1}">
    <br>发生模块：更改合同
    <br>错误信息：房屋价格金额输入有误，金额应大于0元，小于1亿元
</c:if>
<c:if test="${signC_null==1}">
    <br>发生模块：签署合同
    <br>错误信息：关键信息缺失，请检查表单输入是否完整
</c:if>
<c:if test="${signPrice_wrong==1}">
    <br>发生模块：签署合同
    <br>错误信息：房屋价格金额输入有误，金额应大于0元，小于1亿元
</c:if>
